...
Signature BWA per-band edge curves. Nothing else on the market does this.
...
BWA-BandShape is a signature BWA-only plugin. Per-band edge curves let the user surgically shape the boundaries between Bark bands. Combined with the 24-band EQ display, it gives access to a class of processing nothing else exposes.

Polish what's unique. Market as THE BWA secret sauce.
...
BWA-BandShape ships on **two surfaces** — same DSP, different UI:
...
[Releases](https://github.com/vmcguire/BWA-BandShape/releases). Build from
